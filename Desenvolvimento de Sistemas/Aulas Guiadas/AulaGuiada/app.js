...
const { Client } = pg;
const prompt = promptSync(); // Inicializa o prompt para terminal
  ...
  try {
    ...
    if (resultado.rows.length === 0) {
      ...
      resultado.rows.forEach(item => {
        ...
        console.log(` Tipo: ${item.tipo} | Preço: R$ ${precoFormatado} | Estoque: ${item.estoque}`);
...
async function cadastrarItem() {
  try {
    ...
    const resultado = await client.query(
      `INSERT INTO itens (nome, tipo, preco, estoque, descricao) VALUES ($1, $2, $3, $4, $5) RETURNING *`, 
      [nome, tipo, parseFloat(preco), parseInt(estoque || 0), descricao]
    );
